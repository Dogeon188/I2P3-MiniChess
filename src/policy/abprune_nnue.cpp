 ...
 */
std::pair<int, Move> ABPruneNNUE::_get_move(State *state, int depth, int ᶐ, int β, bool isMax) {
    ...
    if (depth == 0) {
        ...
        // use HCE to evaluate
        return std::make_pair(state->evaluateNNUE() * (isMax ? 1 : -1), state->legal_actions.back());
    }
